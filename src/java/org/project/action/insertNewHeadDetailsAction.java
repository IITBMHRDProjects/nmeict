 ...
 * @author asl
 */
public class insertNewHeadDetailsAction extends ActionSupport {
    ...
    public String execute() {
        ...
        if (person == null) {
            ...
            try {
                ...
                head_details.setInstituteId(Integer.parseInt(idInstitute));
                if (head_details.getInstituteId() != 0 && head_details.getStream().getStreamId() != 0) {
                    alreadyPresent = uproc.checkAlreadyHeadDetails(head_details.getInstituteId(), head_details.getStream().getStreamId());
                    if (alreadyPresent > 0) {
                        result = -2;
                    } else {
                        head_details.setLast_updateby(person.getLoginId());
                        result = uproc.addNewHeadDetails(head_details);
                    }
                } else {

                }
                //System.out.println("in Action class" + head_details.getInstituteId() + " s: " + head_details.getStream().getStreamId());
            } catch (Exception e) {
                e.printStackTrace();
            }
            if (result > 0) {
                ...
                session.setAttribute("result", "alreadyPresent");
            }
    ...
    public void validate() {
        if (head_details.getStream().getStreamId() == -1) {
            addFieldError("headStreamError", getText("HeadUpdate.hstream"));
        }
