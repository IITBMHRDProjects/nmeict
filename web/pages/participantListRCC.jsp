<%-- 
    Document   : participantListRCC
    Created on : 15 Sep, 2015, 12:01:50 PM
    Author     : Abinaya
--%>
...
<script type="text/javascript">
    ...
    function checkStatus(workshopid, status) {
        ...
        fetchParticipantDetails5(rcid, workshopid, status);
        fetchParticipantCount(rcid, workshopid, status);
    }
    ...
    function checkStatus3(workshopid, status) {
        rcid = document.getElementById("rcid").value;
        var cc = document.getElementById("cc").value;
        fetchParticipantDetails4(rcid, workshopid, status);
        fetchParticipantCount(rcid, workshopid, status);
    }
    function change() {
...
<br/><br/>
<table class="display1" id="dispatchdetail"  cellpadding="0" cellspacing="0" style="font-size: 110%;width:auto;">
    <thead> 
        <tr>
            <th>S.No</th>
            <th>Enroll No</th>
        ...
        <s:iterator value="participantdetails" status="itStatus">
            <tr class="gradeU">
                ...
                <s:iterator value="workshopContent">
                    ...
                    <s:if test="%{categorycode == 5 || categorycode == 7 || categorycode == 8}">
                        <td><s:property value="remoteCenter.remoteCenterName"/></td>
                        <s:if test='%{categorycode == 8 && facultyStatus.equals("0")}'><td>NA</td></s:if>
                        <s:else><td><s:property value="institute.instituteName"/></td></s:else>
                    </s:if>
                    <td><s:property value="remoteCenter.rc_state" /></td>
                    <% if (callFrom.equals("2")) {%>                
                    <td><s:property value="registrationDetails.accomodation" /></td>
                    <%}%>
