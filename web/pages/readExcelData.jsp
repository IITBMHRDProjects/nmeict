<%-- 
    Document   : readExcelData
    Created on : 15 Jun, 2015, 11:16:27 AM
    Author     : asl
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<%@ taglib prefix="s" uri="/struts-tags" %>
<!DOCTYPE html>
<script>
    function oncheckField() {
         var val = 0;
        document.getElementById("excelreadError").style.display = "none";
        var exfile = document.getElementById("excelread").value;
        if (exfile.trim() == "" || exfile.trim() == null) {
            val++;
            document.getElementById("excelreadError").style.display = "inline";
        } else {
             var resExt=validateLetter(exfile);
             if(resExt == false){
                 val++;
                 document.getElementById("excelreadError").style.display = "inline";
             }
        }

        if (val > 0) {
            return false;
        }
    }

    function validateLetter(letter)
    {
        var result = false;
        var filext = (getExt(letter)).toLowerCase();
        result = ((letter != null) && (letter != "") && (filext == "xls"));
        return result;
    }
    function getExt(filename) {
        var dot_pos = filename.lastIndexOf(".");
        if (dot_pos == -1)
            return "";
        return filename.substr(dot_pos + 1).toLowerCase();
    }
</script>

<body>
    <h3>Invite Users for Registration</h3>
    <p>Use the template for uploading <a href="<s:url value="Excel_Sheets/read_template1.xls"></s:url>" >click here</a></p>
    <p>Workshopid,Remotecenter,Firstname,Lastname,Email,Institute Name, is mandatory</p>
    <div class="form_enclosure">   
        <form action="doreadexcel.html"  method="post" enctype="multipart/form-data" onsubmit="return oncheckField();">
            <div class="form_settings">    
                <div class="form_box">
                    <p>
                        <span>Upload the File :(.xls)</span><sup><img src="images/star.png"></sup>
                        <input type="file" id="excelread"  name="excelread"/>
                        <label id="excelreadError" class="errormsg">Please choose the file (only in .xls)</label>
                    </p><br>
                    <p><input style="position: relative;right: 50px" class="submit" type="submit" name="submit" value="Submit" />
                        <input style="position: relative; right:75px"  class="submit" type="reset" name="reset" value="Reset" onclick=""/>
                    </p> 
                </div>
            </div>
        </form>
    </div>
    <br><br>
    <div>
      
    </div>
</body>
<script>
    function disErr(){
        document.getElementById("suberr").style.display="none";
    }
</script>

