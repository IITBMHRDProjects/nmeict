...
<%
    ...
    String state_enc = request.getParameter("state");
    String contextPath = ServletActionContext.getServletContext().getRealPath("");
    String accepted_encrypted = cipher.encrypt("Accepted");
    String state_value = cipher.decrypt(state_enc);
...
<s:if test='%{#session.loggedInUser.roleId > 1}'>
    ...
    <div>
        ...
        <s:iterator value="workshopContent">
            ...
            <h4 style="color: black;"> <b>Click on the image to export details of the participants</b> <a onclick="createExcel1(<s:property value="workshopid"/>,<s:property value="categorycode"/>, '<%=state_value%>', <s:property value="respectiveCoWorkshopid"/>)" style="position:relative;left:20px;top:10px;" title="Export to Excel">
            ...
            <div class="form_settings">  
                <s:if test="%{categorycode == 5 || categorycode == 7 || categorycode == 8}">
                    <table class="content_container1" style="font-size: 110%;">
                        <tr>
                            <td>State : </td><td><select class="displayselect" id="rcid" name="remotecenter" onchange="changeTest1('<%=update_enc%>', '<s:property value="workshopid_encrypted"/>', '<s:property value="status_encrypted"/>');" >
                                    <option value="<%=all_encrypted%>">All</option>
                                    <s:iterator value="state_list">
                                        <s:if test="%{stateId == state_present}" >
                                            <option value="<s:property value="enc_state"/>" selected="true"><s:property value="stateName"/></option>
                                        </s:if>
                                        <s:else>                                           
                                            <option value="<s:property value="enc_state"/>"><s:property value="stateName"/></option>
                                        </s:else>
                                    </s:iterator>
                                </select>
                            </td>
            ...
            <s:iterator value="countOfParticipants">
                ...
                <s:if test='%{categorycode == 7 }'>
                    <div style="margin-left: 80px;width: 740px;_margin-left:130px;padding:7px;display:inline-block;position: relative;left:150px;">
                        ...
                        <div class="gap20">&nbsp;</div>
                        <div id="WaitlistedParticipants" class="waitlisted tipTip" style="width: 100px; position: relative;left:255px; display: inline;" title="This status means participant has been accepted for the workshop"><a href="participantDetailsSW.html?update=<%=update_enc%>&workshopid=<s:property value="workshopid_encrypted"/>&state=<%=state_enc%>&status=<%=accepted_encrypted%>"   id="statusWaitlisted">Accepted (<s:property value="acceptedParticipants"/>)</a> </div>
                        ...
                        <div class="clear">&nbsp;</div>
                        <div id="AllParticipants" class="all tipTip" style="position: relative;left:35px;" title="Click to see all participants of all status."><a href="participantDetailsSW.html?update=<%=update_enc%>&workshopid=<s:property value="workshopid_encrypted"/>&status=<%=all_encrypted%>&state=<%=state_enc%>"  id="statusAll">All (<script>document.writeln(statusAlljs)</script>)</a></div>
                        <div class="rightarrow" style="position: relative;left:35px;">&nbsp;</div>
                        <div id="RegisteredParticipants" class="requestregistered tipTip" style="position: relative;left:30px;" title="This status means paticipant has registered for the workshop"><a href="participantDetailsSW.html?update=<%=update_enc%>&workshopid=<s:property value="workshopid_encrypted"/>&state=<%=state_enc%>&status=<%=registered_encrypted%>"  id="statusRegistered">Registered (<s:property value="registeredParticipants"/>)</a></div>
                        <div class="rightarrow" style="position: relative;left:35px;">&nbsp;</div>
                        <div id="ApprovedParticipants" style="position: relative;left:30px;" class="requestapproved tipTip" title="This status means participant has been provisionally selected."><a href="participantDetailsSW.html?update=<%=update_enc%>&workshopid=<s:property value="workshopid_encrypted"/>&state=<%=state_enc%>&status=<%=approved_encrypted%>"  id="statusApproved"><!--Verified--> In Process(<s:property value="approvedParticipants"/>)</a></div>
                        <div class="rightarrow" style="position: relative;left:35px;">&nbsp;</div>
                        <div class="confirmed tipTip" style="width:100px;position: relative;left:35px" title="This status means participant has confirmed his/her selection and logged in to moodle."><a href="participantDetailsSW.html?update=<%=update_enc%>&workshopid=<s:property value="workshopid_encrypted"/>&state=<%=state_enc%>&status=<%=confirmed_encrypted%>"  id="statusConfirmed">Confirmed (<script>document.writeln(statusConfirmedjs)</script>)</a></div>
                        ...
                        <div id="InvalidParticipants" class="registrationinvalid tipTip" style="width: 100px" title="This status means participant's registration is invalid. The permission letter format is wrong."><a href="participantDetailsSW.html?update=<%=update_enc%>&workshopid=<s:property value="workshopid_encrypted"/>&state=<%=state_enc%>&status=<%=invalid_encrypted%>" id="statusInvalid">Invalid (<s:property value="invalidParticipants"/>)</a></div>
                        <div class="gap100">&nbsp;</div>  
                        <div id="CancelledParticipants" class="cancelled tipTip" style="width: 100px" title="This status means participant has been cancelled"><a href="participantDetailsSW.html?update=<%=update_enc%>&workshopid=<s:property value="workshopid_encrypted"/>&state=<%=state_enc%>&status=<%=cancelled_encrypted%>"  id="statusCancelled">Cancelled (<s:property value="cancelledParticipants"/>)</a></div>
                        <div class="gap100">&nbsp;</div> 
                        <div id="CompletedParticipants" class="completed tipTip" style="width: 100px" title="This status means participant has completed the course and assignments"><a href="participantDetailsSW.html?update=<%=update_enc%>&workshopid=<s:property value="workshopid_encrypted"/>&state=<%=state_enc%>&status=<%=completed_encrypted%>"  id="statusCancelled">Completed (<s:property value="completedParticipants"/>)</a></div>
                ...
                <s:else>
                    <div style="margin-left: 80px;width: 740px;_margin-left:130px;padding:7px;display:inline-block;position: relative;left:150px;">
                        <div id="AllParticipants" class="all tipTip" style="width: 100px; display:none;" title="Click to see all participants of all status."><a href="javascript:void(0);"  onclick="checkStatus1('<s:property value="workshopid_encrypted"/>', '<%=all_encrypted%>');" id="statusAll">All (<script>document.writeln(statusAlljs)</script>)</a> </div>
                        <div class="gap20">&nbsp;</div>
                        <div id="WaitlistedParticipants" class="waitlisted tipTip" style="width: 100px; position: relative;left:80px; display: none;" title="This status means participant has been waitlisted for the workshop as the seating capacity of the selected remote center is full"><a href="participantDetailsSW.html?update=<%=update_enc%>&workshopid=<s:property value="workshopid_encrypted"/>&state=<%=state_enc%>&status=<%=waitlist_encrypted%>"   id="statusWaitlisted">Waitlisted (<s:property value="waitlistedParticipants"/>)</a> </div>
                        <div style="background-size: 40px 20px;background-position: center;width:80px;position: relative;left:80px; display: none;" class="rightarrow">&nbsp;</div>
                        <div class="clear">&nbsp;</div><div class="gap100">&nbsp;</div> <div class="gap50">&nbsp;</div> <div class="gap10">&nbsp;</div>
                        ...
                        <div id="RegisteredParticipants" class="requestregistered tipTip" style="position: relative;left:30px;" title="This status means paticipant has registered for the workshop"><a href="participantDetailsSW.html?update=<%=update_enc%>&workshopid=<s:property value="workshopid_encrypted"/>&state=<%=state_enc%>&status=<%=registered_encrypted%>"  id="statusRegistered">Registered (<s:property value="registeredParticipants"/>)</a></div>
                        <div class="rightarrow" style="position: relative;left:35px;">&nbsp;</div>
                        <div id="ApprovedParticipants" style="position: relative;left:30px;" class="requestapproved tipTip" title="This status means participant has been provisionally selected."><a href="participantDetailsSW.html?update=<%=update_enc%>&workshopid=<s:property value="workshopid_encrypted"/>&state=<%=state_enc%>&status=<%=approved_encrypted%>"  id="statusApproved">In Process (<s:property value="approvedParticipants"/>)</a></div>
                        <div class="rightarrow" style="position: relative;left:35px;">&nbsp;</div>
                        <div class="confirmed tipTip" style="width:100px;position: relative;left:35px" title="This status means participant has confirmed his/her selection and logged in to moodle."><a href="participantDetailsSW.html?update=<%=update_enc%>&workshopid=<s:property value="workshopid_encrypted"/>&state=<%=state_enc%>&status=<%=confirmed_encrypted%>"  id="statusConfirmed">Confirmed (<script>document.writeln(statusConfirmedjs)</script>)</a></div>
                        <div id="ConfirmedParticipants" class="clear">&nbsp;</div>
                        <div  class="vbar" style="height:40px;width:80px;_width:85px;margin-left: 10px;">&nbsp;</div><div class="blank">&nbsp;</div>
        ...
        <table class="display1" id="participantDetail1" style="word-spacing: normal;text-wrap: avoid;">
            <thead id = "headers" > 
                <tr>
                    ...
                    <th>Name</th>
                    <th>Email</th>
                    <th style="display: none;">RC ID</th>
                    <th style="display: none;">Remote Center Name</th>
                        <s:iterator value="workshopContent">
                            <s:if test="%{categorycode == 5 || categorycode == 7 || categorycode == 8}">
            ...
            <tbody align="center" id="participantDetailsBody1" >
                <s:iterator value="participantdetails" status="itStatus">
                    <tr class="gradeU">
                        ...
                        <td style="display: none;" id="remotecenterId_<s:property value="#itStatus.count" />"><s:property value="remoteCenter.remoteCenterId"/></td>
                        <td style="display: none;" id="remotecenterName_<s:property value="#itStatus.count" />"><s:property value="remoteCenter.remoteCenterName"/></td>
                        <s:iterator value="workshopContent">
                            <s:if test="%{categorycode == 5 || categorycode == 7 || categorycode == 8}">
                        ...
                        <s:if test='%{#session.loggedInUser.roleId != 2}'>
                            <td style="padding:5px 10px;" id="update_<s:property value="#itStatus.count" />">
                                <s:if test="%{stateId == state_present}" >
                                    <button class="approvedButton" onclick="updateReg1('<s:property value="email_enc" />', '<%=workshopid_enc%>')">Update</button>
                                </s:if><s:else>
...
<script>
    function changeTest1(update, workshopid, status) {
        var state = document.getElementById('rcid').value;

        window.location.href = "participantDetailsSW.html?update=" + update + "&workshopid=" + workshopid + "&status=" + status + "&state=" + state;
    ...
    function createExcel1(workshopid, categorycode, state, res_coid) {
        var data = "data=" + workshopid + "+" + categorycode + "+" + state + "+" + res_coid;
        $.ajax({
            url: "pages/DAOCreateExcel_participantDetailsSW.jsp",
            type: "GET",
            data: data,
            dataType: "json",
    ...
    function updateReg1(email, workshopid) {
        window.open("updateDetailsSW.html?email=" + email + "&workshopid=" + workshopid);
    }
</script>
</body>
